...
\begin{tikzpicture}[scale=1.5]
  ...
  % Vectors
  \draw[->, red, thick] (0,0) -- (2.5,1.5) coordinate (v1) node[midway, below right] {$\mathbf{v}_1$};
  \draw[->, blue, thick] (0,0) -- (1.5,2) coordinate (v2) node[midway, above left] {$\mathbf{v}_2$};

  % Angle
  % \pic [draw, <->, angle radius=15mm, angle eccentricity=1.2, "$\theta$"] {angle = v1--(0,0)--v2};

  % Direct line
  \draw[dashed] (v1) -- (v2) node[midway, above right] {Euclidean distance};
\end{tikzpicture}
\end{figure}
