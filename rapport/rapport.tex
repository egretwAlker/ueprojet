...
\subsection{Word Representation Results Comparison}

The table presented in Table \ref{tab:knn_accuracy} provides a comparison of the accuracy results obtained using different word representations and distance measurement techniques in the K-Nearest Neighbors (KNN) algorithm. The table consists of three rows representing the word representation methods: One Hot Encoding, Bag of Words (BoW), and TF-IDF. The columns represent the distance measurement techniques: Euclidean distance and Cosine distance. The table showcases the accuracy values achieved by applying KNN with each word representation and distance combination. It can be observed that KNN with BoW and Cosine distance yields the highest accuracy of 0.693, followed closely by KNN with One Hot Encoding and both Euclidean and Cosine distances. On the other hand, KNN with TF-IDF and Euclidean distance exhibits the lowest accuracy of 0.484. These results provide insights into the performance of different word representation methods and distance metrics in sentiment analysis tasks.

\begin{table}[h]
    \centering
    \caption{KNN Accuracy Using Different Word Representation and Distance Measurement}
    \label{tab:knn_accuracy}
    \begin{tabular}{ccc}
        \hline
        \textbf{Word Representation} & \textbf{Euclidean Distance} & \textbf{Cosine Distance} \\
        \hline
        Bag of Words & 0.685 & 0.694 \\
        TF-IDF & 0.484 & 0.710 \\
        One hot encoding & 0.686 & 0.686 \\
        \hline
    \end{tabular}
\end{table}

Table \ref{tab:knn_accuracy} shows the accuracy results for the K-nearest neighbors algorithm using different word representation techniques and distance measurements.
